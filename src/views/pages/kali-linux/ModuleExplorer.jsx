        ...
        <div className="min-h-[calc(100vh-140px)] flex flex-col gap-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
                <div className="flex items-center gap-4">
                    <button
                        onClick={goBack}
                        className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${viewLevel > 0 ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white hover:scale-105' : 'bg-green-500 text-white'}`}
                        disabled={viewLevel === 0}
                    >
                        {viewLevel === 0 ? <Terminal className="w-5 h-5" /> : <ChevronLeft className="w-6 h-6" />}
                    </button>
                    <div>
                        ...
                        <h1 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white uppercase italic leading-none">
                ...
                <div className="flex items-center gap-3">
                    {viewLevel < 2 && (
                        <div className="relative group">
                            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400 group-focus-within:text-green-500 transition-colors" />
                            <input
                                type="text"
                                placeholder={`Search ${viewLevel === 0 ? 'topics' : 'sub-topics'}...`}
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl pl-10 pr-4 py-2.5 text-sm w-64 focus:border-green-500/50 outline-none transition-all"
                            />
                        </div>
                    )}
                    <button
                        onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')}
                        className="p-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded-xl transition-all text-zinc-500 hover:text-green-500 shadow-sm flex items-center gap-2"
                        title={`Switch to ${viewMode === 'grid' ? 'List' : 'Grid'} View`}
                    >
                        {viewMode === 'grid' ? <List className="w-5 h-5" /> : <LayoutGrid className="w-5 h-5" />}
                    </button>
                    <button
                        onClick={() => { }} // Initialized for Add Logic
                        className="flex items-center gap-2 px-5 py-2.5 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-xl font-bold text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg"
                    >
                        <Plus className="w-4 h-4" /> Add {viewLevel === 0 ? 'Topic' : viewLevel === 1 ? 'Subtopic' : 'Content Item'}
                    </button>
                </div>
            </div>
            ...
            {viewLevel === 0 && (
                viewMode === 'grid' ? (
                    ...
                    <div className="flex-1 overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] shadow-sm flex flex-col">
                        <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50 flex items-center text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">
                            <div className="flex-1">Topic Name & ID</div>
                            <div className="w-48">Status / Environment</div>
                            <div className="w-32 text-right px-4">Actions</div>
                        </div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            {loading ? (
                                Array.from({ length: 10 }).map((_, i) => (
                                    <div key={i} className="h-16 border-b border-zinc-100 dark:border-zinc-800 animate-pulse" />
                                ))
                            ) : filteredTopics.map(topic => (
                                <div
                                    key={topic.id}
                                    onClick={() => handleTopicSelect(topic)}
                                    className="group flex items-center p-4 hover:bg-green-500/5 border-b border-zinc-100 dark:border-zinc-800 transition-colors cursor-pointer"
                                >
                                    <div className="flex-1 min-w-0 ml-4">
                                        <h3 className="font-black text-base text-zinc-900 dark:text-white truncate group-hover:text-green-500 transition-colors">{topic.name}</h3>
                                        <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-tighter opacity-60">{topic.id}</p>
                                    </div>
                                    <div className="w-48">
                                        <span className="px-3 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800 text-[9px] font-black text-zinc-500 uppercase tracking-widest border border-zinc-200 dark:border-zinc-700">Production Node</span>
                                    </div>
                                    <div className="w-32 flex items-center justify-end gap-2 px-4">
                                        ...
                                        <ChevronRight className="w-5 h-5 text-zinc-300 group-hover:translate-x-1 transition-transform ml-2" />
                                    </div>
                                </div>
                            ))}
                        </div>
